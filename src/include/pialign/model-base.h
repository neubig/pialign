...
public:
    ...
    virtual void addGen(WordId jId, const Span & mySpan, Prob prob, int* tCounts, WordString & jIds) = 0;

    // add a count to the model from the base distribution
    virtual void addBase(WordId jId, const Span & mySpan, Prob, int* tCounts, WordString & jIds) = 0;

    // add a count to the model that was generated by splitting into two trees
    virtual void addTree(WordId jId,WordId lId,WordId rId,const Span & jSpan,const Span & lSpan, const Span & rSpan, int type, Prob prob, int* tCounts, WordString & jIds) = 0;

    // remove a sentence sample from the distribution
    // counts contains the count of 4 types (input is a pointer to an array containing reg, inv, term, null counts)
    virtual void removeSentence(WordId head, WordString & jIds, int* counts) = 0;
    ...
    virtual void checkEmpty() const {
        if(termOrNot_.getTotal() > 0)
            std::cerr << "WARNING: non-empty termOrNot" << std::endl;
        if(forBack_.getTotal() > 0)
            std::cerr << "WARNING: non-empty forBack" << std::endl;
        if(phrases_.getTableCount() > 0)
           std::cerr << "WARNING: non-empty phrases" << std::endl;
    }
