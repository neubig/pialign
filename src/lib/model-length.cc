...
Prob LengthModel::addSentence(const WordString & e, const WordString & f, SpanNode* node, StringWordSet & ePhrases, StringWordSet & fPhrases, PairWordSet & pairs, std::vector<Prob>& baseProbs) {
    ...
    if(toAdd == TYPE_REG || toAdd == TYPE_INV) {
        node->prob += addSentence(e,f,node->right,ePhrases,fPhrases,pairs,baseProbs);
        node->prob += addSentence(e,f,node->left, ePhrases,fPhrases,pairs,baseProbs);
...
SpanNode* LengthModel::removePhrasePair(WordId jId, std::vector<Prob>& baseProbs) {
    if(jId < 0) return 0;
#ifdef DEBUG_ON
